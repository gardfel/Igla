...
class Rocket(object):
    ...
    def navigation(self, *args):
        ...
        nu_kr = ((self.S_kr / (self.L_kr * self.b_0_kr) - 0.5) ** (-1) / 2)  # относительное сужение крыльев
        ...
        b_kr = self.b_0_kr * (1 - (nu_kr - 1) / nu_kr * self.d_korm / self.L_kr)  # бортовая хорда крыльев
        S_k_kr = self.S_kr * (1 - ((nu_kr - 1) / (nu_kr + 1)) * self.d_korm / self.L_kr) * (
                1 - self.d_korm / self.L_kr)  # площадь консолей крыльев
        l_k_kr = l_kr * ((1 - self.d_korm / self.L_kr) / (
                1 - ((nu_kr - 1) / (nu_kr + 1) * self.d_korm / self.L_kr)))  # относительное удлинение консолей
        L_k_kr = self.L_kr - self.d_korm  # размах консолей крыльев
        nu_k_kr = nu_kr - self.d_korm / self.L_kr * (nu_kr - 1)
        ...
        nu_op = ((self.S_op / (self.L_op * self.b_0_op) - 0.5) ** (-1) / 2)  # относительное сужение крыльев
        b_1_op = self.b_0_op / nu_op  # концевая хорда крыльев
        b_a_op = 4 / 3 * self.S_op / self.L_op * (1 - (nu_op / (nu_op + 1) ** 2))  # САХ
        z_a_op = self.L_op / 6 * ((nu_op + 2) / (nu_op + 1))  # расстояние от САХ до оси ЛА
        b_op = self.b_0_op * (1 - (nu_op - 1) / nu_op * self.d / self.L_op)  # бортовая хорда крыльев
        S_k_op = self.S_op * (1 - ((nu_op - 1) / (nu_op + 1)) * self.d / self.L_op) * (1 - self.d / self.L_op)  # площадь консолей крыльев
        l_k_op = l_op * (
                    (1 - self.d / self.L_op) / (1 - ((nu_op - 1) / (nu_op + 1) * self.d / self.L_op)))  # относительное удлинение консолей
        L_k_op = self.L_op - self.d  # размах консолей крыльев
        nu_k_op = nu_op - self.d / self.L_op * (nu_op - 1)
        ...
        W_nos = 0.000038389 + 1 / 3 * kk.pi * (self.L_nos - 0.022) * ((self.d / 2) ** 2 + self.d / 2 * 0.031 + 0.031 ** 2)
        print("W_nos", W_nos)
        ...
        D_kr = self.d / self.L_kr  # относительный диаметр корпуса
        D_op = self.d / self.L_op  # относительный диаметр корпуса
        ...
        S__op = self.S_op / (kk.pi * (self.d ** 2) / 4)  # относительная площадь передних несущих поверхностей
        S__kr = self.S_kr / (kk.pi * (self.d ** 2) / 4)  # относительная площадь задних несущих поверхностей
        ...
        while (rast_min >= 5) and (t <= 16) and ((rast_krit - rast_min) <= 200):
            ...
            if mach > 1:
                b__b_op = b_op / (kk.pi / 2 * self.d * kk.sqrt(mach ** 2 - 1))
                b__b_kr = b_kr / (kk.pi / 2 * self.d * kk.sqrt(mach ** 2 - 1))
                L__hv_op = L_hv_op / (kk.pi * self.d * kk.sqrt(mach ** 2 - 1))
                L__hv_kr = L_hv_kr / (kk.pi * self.d * kk.sqrt(mach ** 2 - 1))
            ...
            if jam <= cy1_alf_op * self.alf * kk.pi / 180 * self.v ** 2 * self.d ** 2 * kk.pi / 8 *  Tabl.tab_atm(self.y, 4):
                jam = cy1_alf_op * self.alf * kk.pi / 180 * self.v ** 2 * self.d ** 2 * kk.pi / 8 *  Tabl.tab_atm(self.y, 4)
...
f_t = 0.045216  # площадь поверхности до источника турбулизации
...
vertel = Target(3000, 3500, 320)
igla = Rocket(vertel.y, vertel.x, 0.146, 0.0146, 0.12, 0, 0.25, 0.00786, 0.033, 0, 0.0974, 0.050, 0.160)
print(igla.navigation(vertel))
