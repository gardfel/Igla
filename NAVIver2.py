...
class Rocket(object):
    ...
    def __init__(self, *args):  # 0 - коорд у цели, 1 - коорд х цели
        ...
        # крылья:
        self.L_k_kr = par_kr[0]
        self.S_k_kr = par_kr[1]
        self.b_kr = par_kr[2]
        self.tan_1_kr = par_kr[3]
        self.L_hv_kr = par_kr[4]  # Расстояние от конца бортовой хорды крыла до кормового среза
        print(self.L_hv_kr)

        # рули:
        self.L_k_op = par_op[0]
        self.S_k_op = par_op[1]
        self.b_op = par_op[2]
        self.tan_0_op = par_op[3]
        self.L1_op = par_op[4]  # Расстояние от носика корпуса до серидины бортовой хорды оперения

        # нососвая часть:
        self.L_nos = par_korp[0]
        # кормовая часть:
        self.d_korm = par_korp[1]
        self.L_korm = par_korp[2]
        #
    ...
    def navigation(self, *args):
        ...
        z_a_k_k = self.L_k_kr / 6 * (nu_k_kr + 2) / (nu_k_kr + 1)
...
def krit_maxi(m_0_w, Sfy, Sfx, dealf):
    k_paramz = 0
    return (m_0_w / m_0_w_0) ** (0) * (Sfy / pp2[k_paramz]) ** (2) * (Sfx / pp1[k_paramz]) ** (-4) * (dealf / pp3[k_paramz]) ** (-1)
...
parabellum = np.array([0.146, 0.0146, 0.12, 0, 0.015,
                       0.25, 0.00786, 0.033, 0, 0.35,
                       0.0974,
                       0.050, 0.160])
params_kr = np.array([0.096, 0.008912, 0.10568, 0, 0.015])
params_op = np.array([0.17824, 0.0055380742, 0.03214, 0, 0.35])
params_korp = ([0.0974, 0.050, 0.160])
igla = Rocket(vertel.y, vertel.x, params_kr, params_op, params_korp)
